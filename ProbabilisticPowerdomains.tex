...
\tob{Todo: this shouldn't be an axiom, since the well-ordering principle is constructively equivalent to induction, and second-order induction is part of the definition of a natural numbers object. So instead the axiom should probably be that a natural numbers object $\tNN$ exists}
...
\begin{axiom}
	...
	\[
		\At{K}\exists x:X. P(x) \quad \Longrightarrow \quad \At{K} \bot \: \lor \: \exists x:X.\At{K}P(x).
	\]
\end{axiom}
...
The notion of flabbiness is defined in \cref{def:flabby}.
...
\begin{proof}
	This is similar to the proof of \cref{lem.11_at_flabby}. 
	Let $P : \tNN \to \Prop$ satisfy $\At{K} \exists n : \tNN \ldotp P(n)$. We now use induction to show that we can assume $\forall n : \tNN \ldotp P(n) \imp \At{K} \bot$. The base case is clear, since $\At{K} P(0)$ proves the $\At{K}$-flabbiness claim, and hence we can assume $P(0) \imp \At{K} \bot$. For the induction step, we are again done if $\At{K} P(n+1)$, and can therefore assume $P(n+1) \imp \At{K} \bot$ (the induction assumption is not needed). 
	\[
		\At{K} ( \exists n : \tNN \ldotp P(n) \land \forall m \ldotp P(m) \imp (m \geq n) ).
	\]
	Using this $n$, the claim easily follows from the subsingleton assumption on $P$, since then any other $m:\tNN$ with $P(m)$ must satisfy $m = n$.
\end{proof}
...
\begin{lemma}
	For every $a,b\in\tLR$,
	\[
		\left(\forall \epsilon : \tQQ\ldotp (\epsilon > 0) \imp (a \leq b + \epsilon)\right) \quad \Longrightarrow \quad \left( a \leq b \right).
	\]
	\label{lem:eps_order}
\end{lemma}

\begin{proof}
	We need to prove that $\epsilon < a$ implies $\epsilon < b$. We can choose $\epsilon' \in \tQQ$ such that $\epsilon < \epsilon' < a$. Applying the assumption to $\epsilon' - \epsilon > 0$, we have $a \le\epsilon b + \epsilon' - \epsilon$. Hence $\epsilon' < a$ implies $\epsilon' < b + \epsilon' - \epsilon$, or equivalently $\epsilon < b$, as was to be shown.
\end{proof}
...
\begin{lemma}
	For all $a : \tLR$,
	\[
		\sup_{q \: : \: \At{K} q \leq \At{K} a} \At{K} q = \At{K}a = \inf_{q \: : \: \At{K} a \leq \At{K} q} \At{K} q,
	\]
	where $q\in \tQQ$ ranges over all rationals satisfying the additional constraints.
	\label{lem:at_lower_real}
\end{lemma}

\tob{In the end we don't need this for \cref{prop.order_canc}}

\begin{proof}
	For the first equality, we need to show that if $q' : \tQQ$ is such that $q' < \At{K}a$, then there is some $q : \tQQ$ with $\At{K} q \leq \At{K} a$ and $q' < \At{K} q$. For this, we can take any $q$ with $q' < q < \At{K}a$.

	For the second equality, we need to show that if $q' : \tQQ$ is such that $q' < \At{K} q$ for every $q$ with $\At{K} a \leq \At{K} q$, then also $q' < \At{K}a$. Now $q' < \At{K} q$ is equivalent to $\At{K}(q' < q)$ by definition, and therefore to $\At{K}\bot \:\lor\: (q < q')$ by \cref{lem:rational_compare_at}. Thus we need to show that if $q'$ satisfies $\forall q\ldotp (\At{K} a \leq \At{K} q) \imp (\At{K} \bot \lor (q' < q))$, then also $q' < \At{K} a$. Now by \cref{lem:at_totally_ordered1}, we can assume $\At{K} a \leq \At{K} q'$ without loss of generality; but then the assumption gives $\At{K}\bot \:\lor\: (q' < q')$, or equivalently $\At{K}\bot$, which implies $\At{K}(q' < a)$ as desired.
\end{proof}

%Moreover, we write $a < b$ if there is $q\in\tQQ$ such that $q < b$ and $\lnot(q < a)$. Standard arguments show that these relations are transitive, and that $(a < b) \imp (a \leq b)$.
...
%\begin{proof}
%	Suppose that $(a > b) \imp \At{K}\bot$, and suppose that we have $\At{K}(q < a)$.  UNFINISHED
%\end{proof}
...
\[
	\At{K} ( P \vee Q ) \: \Longleftrightarrow \: \At{K} P \vee \At{K} Q.
\]
\end{lemma}

\begin{proof}
	The nontrivial implication is left to right, so assume $\At{K} ( P \lor Q )$. We first show that the type $1 + 1 = \{\alpha,\beta\}$ is flabby. So let $R \subseteq 1 + 1$ be a subsingleton. In this case, the subsingleton condition is equivalent to $\lnot (R(\alpha) \land R(\beta))$.

, or equivalently $((P \imp (\down K)^c) \wedge (Q \imp (\down K)^c)) \imp (\down K)^c$. By \cref{lem.prime}, this means that one of $P \imp (\down K)^c$ or $Q \imp (\down K)^c$ implies $(\down K)^c$, as was to be shown.
\end{proof}
...
Since every join can be decomposed into a directed join of finite joins, by \cref{lem.regularly_based} an arbitrary proposition $P:\Prop$ is a directed join of elements in $T$. Thus it suffices to show that $T$ is closed under directed joins, but this is clear: if $(Q_d)_{d\in D}$ is a directed system where $Q_d\in T$ for all $d$, then
...
\begin{definition}\label{def.interior}
	If $(X,\Op(X))$ is an internal space and $P : X \to \Prop$ is a predicate, define its \emph{interior} $\int(P)\colon X\to\Prop$ as follows:
	\[
...
\begin{definition}
	Let $K : \CB$. A type $X$ is \emph{$\At{K}$-flabby} if for all $P : X \to \Prop$,
	%\[
	...
	%\]
	\[
		\left( \At{K} \exists x : X \ldotp P(x)\right) \quad \Longrightarrow \quad \At{K}\bot \: \lor \: \left[ \subsing{P} \imp \exists x : X \ldotp (\exists y : X \ldotp P(y)) \imp \At{K} P(x)  \right].
	\]
\end{definition}
...
\begin{proposition}
	...
	In our intended semantics of sheaves no $\CB$, a sheaf $X$ is $\At{K}$-flabby if only if every germ at $K$ can be extended to a global section.
\end{proposition}

\tob{Carefully check the proof again}

\begin{proof}
	The semantics of this internal condition on some open $U$ is as follows. If $K\not\in U$, then it is trivially true, so assume $K\in U$. Then the condition is that if $P$ a predicate on $X|_U$, then there is $U' \subseteq U$ such that $U'$ contains at least all $K'\supseteq K$, and if $U'\models \subsing{P}$ and $U' \models \At{K} \exists x : X \ldotp P(x)$, then also $U' \models \exists x : X \ldotp (\exists y : X \ldotp P(y)) \imp \At{K} P(x)$. The assumption $U' \models \subsing{P}$ means that for every $V\subseteq U'$, if $x,x'\in X(V)$ and $P(x) = P(x') = V$, then $x = x'$. The assumption $U \models \At{K} \exists x : X\ldotp P(x)$ means that there is an open $W\subseteq U$ with $K\in W$ and $\tilde{x} \in X(W)$ with $P(\tilde{x}) = W$. The conclusion means that there is a cover $U' = \bigcup_i U_i$ and local sections $x_i \in X(U_i)$ such that for every $V_i \subseteq U_i$ which contains $K$, if $P$ contains a germ of $X$ at every point of $V_i$, then also $K\in P(x)$.

	We prove this implication from the possibility of lifting germs at $K$ to global sections. The first assumption gives us a germ at $K$, which we extend to a section $\hat{x}\in X(U)$. For the given cover $U' = \bigcup_i U_i$, we put $x_i := \hat{x}|_{U_i}$. Now we are given the $V_i \subseteq U_i$, and need to assume that $P$ contains a germ at every point of $V_i$. For $K\in V_i$, the subsingleton assumption implies that all of these germs are equal to the restriction of $x_i$. In particular, the germ at $K$, which is the restriction of $x$, satisfies $P$ in a neighborhood of $K$; hence $K\in P(x)$, as was to be shown.

	Conversely, suppose that the internal sentence holds. Now it is enough to show that every germ at $K$ can be lifted to a germ at a given $K'\supseteq K$; for then, Zorn's lemma shows that the given germ can be extended to an open which contains all these $K'$; but then the only such open is the whole space $\CB$, resulting in a global section. So hence let $x \in X(U)$ be a representative of a germ defined on an open $U$ with $K \in U$. We instantiate the internal assumption on the open $\CB$. We define a subsingleton as the natural transformation $P : X \to \Prop$ with components $P_V(x') := \bigcup \{V' \subseteq V \: : \: x|_{V'} = \hat{x}|_{V'} \}$, which is clearly natural; also the subsingleton condition holds by construction. Also the other assumption $\CB \models \At{K} \exists x : X \ldotp P(x)$ holds by construction. Thus at every $K' \supseteq K$, we obtain a germ $x'$ defined on a neighborhood $U' \ni K'$ which necessarily also contains $K$. Since $U'\models (\exists y : X \ldotp P(y) ) \imp \At{K} P(x')$, we conclude $P(x') \ni K$ by restriction to the neighborhood of $K$ on which $x$ is defined. Restricting to the intersection of the two domain of definition of $x$ and $x'$, we conclude that $x = x'$ holds there. Hence $x'$ extends the germ represented by $x$, as was to be shown.
\end{proof}
...
\begin{proof}
	Let us write $1 + 1 = \{\alpha,\beta\}$ in terms of elements. Let $P : 1 + 1 \to \Prop$. In case $\At{K} P(\alpha)$, we use $x := \alpha$ and there is nothing more to prove; similarly $x := \beta$ if $\At{K} P(\beta)$. Hence we can assume $P(\alpha) \imp \At{K} \bot$ and $P(\beta) \imp \At{K} \bot$. But now we also have the assumption $\At{K}(P(\alpha) \lor P(\beta))$, and therefore obtain $\At{K} \bot$, as was to be shown.
\end{proof}
