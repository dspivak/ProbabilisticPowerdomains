...
Note that $\Span\set$ is the topos of sheaves on a topological space $W$ with three points $a,b,ab$ and four open sets: $\emptyset$, $W$, $\{a\}$ and $\{b\}$. The type $\tRR$ corresponds to the constant sheaf, by general theory of real number objects in toposes corresponding to topological spaces. As in TTT, the real number objects for the modalities $\At{}$ and $
