...
\begin{example}
The type $\RR$ of real numbers has a poset structure given by $r\leq r'\iff\forall(q:\QQ)\ldotp(q<r)\imp(q<r')$. Here $q<r$ is shorthand for ``$q$ is in the lower cut of $r$''.
\end{example}
...
\begin{example}
The poset $(\RR,\leq)$ has binary joins (given by $\max$) and binary meets (given by $\min$) and it has directed joins (given by $\sup$), but it does not have a bottom or top element.
\end{example}
...
Suppose $(X,\leq)$ has valuations. A \emph{valuation} on $X$ is a function $\mu:X\to\RR$ satisfying the following:
\begin{enumerate}
