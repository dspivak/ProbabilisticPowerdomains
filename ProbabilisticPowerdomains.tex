...
For the other direction, we start with $\mu'$ and construct a $\mu$ from it. Concretely, we put\todo{Here, I want to identify $\Prop$ with the frame of opens of $C\BaseSpace$ considered as an internal space. Does this make sense? If so, then it will have to be proven that this ``set'', in the sense of a predicate $C\BaseSpace\to\Prop$, actually corresponds to an open subobject and therefore to a proposition}
\[
