...
Let's consider the even simpler case where $\Church{X}(\{a\}) = A$ and $\Church{X}(\{b\}) = B$ for \emph{finite} sets $A$ and $B$. Then a measure consists of a measure at $\{a\}$, one at $\{b\}$, and one at $\{a,b\}$. Then the above compatibility condition seems to be saying that the measure at $\{a,b\}$, which is $\mu_{\{a,b\}} : (X \to \tRRat{\{a,b\}})\to\tRRat{\{a,b\}}$, already determines the other two via marginalization; this is exactly what we would hope to obtain. Thus $\Church{X\to\tRRat{\{a,b\}}}\{a,b\}$ is precisely the set of functions $A\times B\to\RR$, while there is only one section locally at $\{a\}$ and $\{b\}$, respectively. Similarly, $\Church{(X\to\tRRat{\{a,b\}})\to\tRRat{\{a,b\}}}\{a,b\}$ is the set of functions $(A\times B\to\RR)\to\RR$, while there is again only one section at each point. The linearity and positivity requirements are indeed the ones that one would expect, saying that $\Church{\mu}\{a,b\}:(A\times B\to\RR)\to\RR$ needs to be linear and positive.
