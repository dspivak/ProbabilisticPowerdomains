...
Now given an internal valuation $\mu$ on $\Omega^X$, we get an external valuation $\mu_K$ on the set $2^{X_K}$, where $X_K$ is the stalk of $X$ at $K$. Indeed, for any subset $T\ss 2^{X_K}$, it is given by the formula
...
\[\mu(T_K)(K)\leq\mu(\rho_{K,K'}\inv(T_K))(K)\leq\mu(\rho_{K,K'}\inv(T)_{K'})(K')\]
