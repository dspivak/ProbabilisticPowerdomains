...
In other words, we only equip standard first-order intuitionistic type theory with an additional system of modalities $\{ \At{K} \: : \: K\in \CB\}$ indexed by some type $\CB$. \dis{Do we need $\CB$ to have an order or anything? Aren't we using that $\At{K}$ means $((-\imp k)\imp k)$ for some  $k:\Prop$?}
