...
\begin{proposition}[{\cite[Proposition~3.3]{Edalat:1995a}}]
If $\BaseSpace$ is a locally compact Hausdorff space, then its upper space $C\BaseSpace$ is a domain; in particular its topology coincides with the Scott topology. The ordering on points is given by $K_\CB\sqss K'_\CB$ iff $K'\ss K$, and the way-below relation $K_\CB\ll K'_\CB$ holds iff there is an open set $U$ with $K'\ss U\ss K$.
\end{proposition}
...
\[
	K \longmapsto \exists U \ldotp \At{K} U \wedge \mu'(\mathtt{Const}(\In{U} P))(q),
\]
