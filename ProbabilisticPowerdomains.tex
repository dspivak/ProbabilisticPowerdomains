...
In other words, we only equip standard first-order intuitionistic type theory with an additional system of modalities $\{ \At{K} \: : \: K\in \CB\}$ indexed by some fixed type $\CB$.
...
\begin{proof}
	The left-to-right implication is straightforward and holds for every modality. For the right-to-left implication, we can assume $P \imp \At{K} \bot$ by the decidability \cref{ax.atBoolean}. But then we get $\At{K} \bot$ by assumption, which is enough.
\end{proof}
