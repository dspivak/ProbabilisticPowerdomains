...
\begin{definition}
We say that a poset $(X,\leq)$ is a \emph{frame} \todo{Check that this is equivalent to the definition of internal frame}if it has binary meets, binary joins, and directed joins.
...
\end{enumerate} \todo{is $\sup : \subtype{\const{Drct}}\to X$ well-defined? We've only defined what a sup is for each particular directed subtype, without talking about uniqueness}
...
\begin{definition}
Let $X$ be a type. For $t:\Time$ and $d,u\in\tRR$, a \emph{measure on $X$ at $[d,u]$} is defined to be a function $\mu:(X\to\tRRat{[d,u]})\to\tRRat{[d,u]}$%
\footnote{
...
The things that we want to take expectation values of should be \emph{observables} on $X$. In the sheaf of observables, a section over $(a,b)$ should be a function $:\prod_{(c,d)\subseteq(a,b)} X(c,d)\to\II\RR$ which is suitably monotone, i.e.~such that \ldots
