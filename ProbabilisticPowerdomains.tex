...
Using the fact that the inverse image functor $f^*$ preserves finite limits, it's straightforward to see that if $\mathcal{O}$ is a frame in $\mathcal{T}'$, then $f^*\mathcal{O}$ is a meet-semilattice in $\mathcal{T}$. However, the existence of arbitrary joins for $I : f^*\mathcal{O}\to\Prop$ is not clear. How does $f^*\mathcal{O}\to\Prop$ relate to $\Prop'\in\mathcal{T}'$? Presumably there is some comparison map between $\Prop'$ and $f_*\Prop$?
