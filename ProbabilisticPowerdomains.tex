...
Our first axiom is concerned with the double negation modality, while all following axioms will involve $\At{}$.
...
\chapter{Valuations and dense subtoposes}

\tob{``dense'' is wrong. TODO: Figure out what this should be instead}

\begin{conjecture}
Let $j$ be a dense modality. Then for every type $X$, valuations $\Prop^X \to \tLR$ are in canonical bijection with valuations $\Prop_j^{\asSh_j{X}} \to \tLR_j$.
\end{conjecture}

\tob{Are the lower $j$-reals isomorphic to the $j$-sheafification of the lower reals? If not, I'm not sure which one of the two I want}

Of course we would want this to hold internally.

We write $\eta_j : X \to \asSh_j{X}$ for the unit of the sheafification adjunction.

Given $\mu : \Prop^X\ \to \tLR$, we construct $\mu' : \Prop_j^{\asSh_j{X}} \to \tLR_j$ as follows. Let $P : \asSh_j{X} \to \Prop_j$ be a predicate. Then
\[
	\mu'(P)(q) := j \mu(P\circ \eta_j)(q).
\]
Conversely, given $\mu'$ we can try to construct $\mu$ as
\[
	\mu(P) := \sup_{jQ \implies P} \mu'(jQ),
\]
meaning that
\[
	\mu(P)(q) = \exists Q. (jQ\implies P) \land \mu'(jQ)(q).
\]
So in order for the two constructions to be inverses of one another, we need in particular that
\[
	\mu(P)(q) = \exists Q. (jQ\implies P) \land j\mu(jQ\circ \eta_j)(q)
\]
