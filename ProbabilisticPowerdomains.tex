...
\begin{proof}
	...
	Hence we get $\At{K}(\ell + \varepsilon n) \leq \At{K}(q + u)$. Making $n$ large enough gives rationals $q_-,q_+\in \tQQ$ with $q_- < q_+$ but $\At{K} q_+ \leq \At{K} q_-$, and therefore $\At{K}(q_- < q_-)$, or $\At{K}\bot$, which is enough.
\end{proof}
...
\begin{lemma}
	\label{lem.mu_at_Q}
	For every internal space $(X,\Op(X))$ and valuation $\mu : \Op(X) \to \tLR$, we have $\At{K} \mu(Q) = 1$ for every $Q : \Op(X)$ which satisfies $\At{K} Q$.
\end{lemma}

\begin{proof}
	Since $\At{K} Q$ by assumption, we also have
	\begin{equation}
		\label{approx_Q}
		\exists U : X \to \Prop \ldotp \At{K} U \land (U \imp Q)
	\end{equation}
	by \Cref{prop.at_to_nghbhd}. Now consider $D : \Op(X) \to \Prop$ given by
	\[
		D(V) \: := \: \left( V \imp \exists U : X \to \Prop \ldotp \At{K} U \land (U \imp Q) \right).
	\]
	Then it is easy to see that \eqref{approx_Q} is equivalent to $\exists V : \Op(X) \ldotp D(V) \land V$. Since $D$ is by definition a (the predicate associated to) a principal downset in a lattice, it follows that $D$ is directed. Hence we have
	\[
		\exists V : \Op(X) \ldotp D(V) \land \mu(V)(q) \: = \: (q < 1)
	\]
	for all $q : \tQQ$.

	We now show that $D$ is directed. Since $D$ holds on \eqref{approx_Q} itself, we know that $D$ is inhabited. If $D(V_1)$ and $D(V_2)$, then 

	We have, for every $q : \tQQ$ with $q \geq 0$,
	\[
		\At{K} \mu(Q)(q) = \At{K} \exists U : X \to \Prop \ldotp \At{K} U \land (U \imp \mu(Q)(q)),
	\]
	which follows trivially upon 
	\[
		\At{K} \mu(Q)(q) = \At{K} \mu(\At{K} \bot \lor Q).
	\]
...
\begin{proof}
	...
	\end{align*}
	Thus upon hitting both equations with $\At{K}$, the claim follows from the cancellativity of \Cref{prop.at_order_canc} if we can prove that $\At{K} \mu(P \lor (P \imp \At{K} \bot)) = \At{K} 1$. But this is now a consequence of \Cref{lem.mu_at_Q}.
\end{proof}
