...
\begin{axiom}\label{lem:at_disjunction}
...
For the other direction, we start with $\mu'$ and construct a $\mu$ from it. Concretely, we consider the predicate $CB \to \Prop$ given by
\[
	K \mapsto \sup \mu'(\At{B} \In{U} P)(q).
\]
and claim that it comes 
\[
	\mu(P)(q) \coloneqq \left\{ K \:\Big|\: \sup_{U\ni K} \mu'(\At{B} \In{U} P)(q) \right\}. 
\]
\todo[inline]{Check that this has the required properties}

We now verify that these two constructions are inverse of one another. Starting with $\mu'$, we need to prove that for all $\At{B}$-closed predicates $P$,
\[
	\mu'(P)(q) = \At{B} \left\{ K \:\Big|\: \sup_{U\ni K} \mu'(\At{B} \In{U} P)(q) \right\}
\]
This is easy:
\begin{align*}
