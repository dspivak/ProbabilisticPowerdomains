...
Hence by \Cref{lem.sup_vs_directed} and the fact that both $\mu$ and $i$ preserve directed suprema, it is enough to show that $T$ is closed under binary disjunctions. So suppose that we are given $Q_1,Q_2\in T$. Then
\begin{align*}
	...
	&=i(Q_1)+i(Q_2).
\end{align*}
Since $i(Q_1\vee Q_2)+i(Q_1\wedge Q_2)=i(Q_1)+i(Q_2)$, it follows that all the above inequalities must be equalities. By the cancellativity of \Cref{prop.order_canc}, this results in particular in $\mu(Q_1\vee Q_2)=i(Q_1\vee Q_2)$.
\end{proof}
