...
\subsection{Semantics for finite $\pi$-types}

Let's say that a $\pi$-type $X$ is \emph{finite} if every $\Church{X}(a,b)$ is finite. What is the semantics of measures on finite $\pi$-types?
...
\subsection{Good in the case of finite sets}

Now let's work what a measure on a finite $\pi$-type $X$ is. By finite, we mean that $\Church{X}(\{a\}) = A$ and $\Church{X}(\{b\}) = B$ for \emph{finite} sets $A$ and $B$. Then a measure consists of a measure at $\{a\}$, one at $\{b\}$, and one at $\{a,b\}$. Then the above compatibility condition says that the measure at $\{a,b\}$, which is $\mu_{\{a,b\}} : (X \to \tRRat{\{a,b\}})\to\tRRat{\{a,b\}}$, already determines the other two via marginalization\todo{can we prove that internally?}; this is exactly what we would hope to obtain. Thus $\Church{X\to\tRRat{\{a,b\}}}\{a,b\}$ is precisely the set of functions $A\times B\to\RR$, while there is only one section locally at $\{a\}$ and $\{b\}$, respectively. Similarly, $\Church{(X\to\tRRat{\{a,b\}})\to\tRRat{\{a,b\}}}\{a,b\}$ is the set of functions $(A\times B\to\RR)\to\RR$, while there is again only one section at each point. The linearity and positivity requirements are indeed the ones that one would expect, saying that $\Church{\mu}\{a,b\}:(A\times B\to\RR)\to\RR$ needs to be linear and positive.
...
\chapter{Some notes on locales}

Tobias: these are notes to aid my own understanding.

\section{Domain-theoretic aspects}

This stuff is taking place in $\Cat{Set}$.
...
Using the fact that the inverse image functor $f^*$ preserves finite limits, it's straightforward to see that if $\mathcal{O}$ is a frame in $\mathcal{T}'$, then $f^*\mathcal{O}$ is a meet-semilattice in $\mathcal{T}$. However, the existence of arbitrary joins for $I : f^*\mathcal{O}\to\Prop$ is not clear. How does $f^*\mathcal{O}\to\Prop$ relate to $\Prop'\in\mathcal{T}'$?

Note that there is a natural comparison map between $f_*\Prop$ and $\Prop'$. Indeed, since $f_*$ preserves monos, there is a map $\classify{f_*\const{true}}\colon f_*\Prop\to\Prop'$ classifying the image under $f_*$ of $\const{true}\colon 1\to \Prop$. Thus we obtain a comparison map $\mathcal{O}\to f_*\Prop\to\Prop'$.

The image $f_*\Prop$ is an internal frame in $\cat{T}'$; see \cite[Intro to C.1.6]{Johnstone:2002a}. It doesn't seem that $f^*\Prop'$ will be an internal frame in general. For example, why should $P,Q\colon 1\to f^*\Prop'$ have a meet? There is a map $f^*(\wedge)\colon f^*\Prop'\times f^*\Prop'\to f^*\Prop'$, but I don't see why it should define a meet operation in $\cat{T}$. According to \cite[C.1.6 and Corollary B.3.7]{Johnstone:2002a}, if $f^*$ has a left adjoint that preserves pullbacks then $f^*\Prop'$ will be an internal frame. See also \href{https://mathoverflow.net/questions/34433/an-elementary-question-about-adjunctions-between-presheaf-categories-preserving}{this mathoverflow question}.
