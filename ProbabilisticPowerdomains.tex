...
\begin{proof}
	By \Cref{lem.at_plus_commute} and $q \leq \At{K} q$, it is enough to prove the $\leq$ inequality. Now if $q' < \At{K}(a + q) = \At{K} a + \At{K}q$, then by \eqref{eq.cancel_at_q'}, we get $q' - q < \At{K} a$, which implies the desired $q' < \At{K} a + q$.
\end{proof}
...
\begin{proof}
	By \Cref{prop.N_or_vs_forall}, we have 
	\[
	...
	\]
	In the second case, note that the implication $(q < a) \imp \At{K} (q < b)$ is already $\At{K}$-closed, and we can therefore assume it to hold. Hence the desired inequality $\At{K} a \leq \At{K} b$ is implied.
...
\begin{proof}
	...
	Now suppose that we have $q : \tQQ$ with $q < \At{K}a$, and want to show $q < \At{K}b$. By \Cref{lem:at_totally_ordered1}, we can assume $\At{K} b \leq \At{K} q$. Since we already have $\At{K} c \leq \At{K} u$, we simplify notation by assuming $q = u$ without loss of generality, so that $\At{u}$ is an upper bound for both $\At{K}b$ and $\At{K}c$, and therefore $2\,\At{K}u$ is an upper bound for $\At{K}(b + c)$. Since every lower real has a rational lower bound, we can also find $\ell : \tQQ$ with $2 \At{K} \ell \leq b + c$.

	Hence we get $\At{K}(2\ell + \varepsilon n) \leq \At{K}(2u)$. Making $n$ large enough gives rationals $q_-,q_+\in \tQQ$ with $q_- < q_+$ but $\At{K} q_+ \leq \At{K} q_-$, and therefore $\At{K}(q_- < q_-)$, or $\At{K}\bot$, which is enough.
\end{proof}
...
\begin{proof}
	...
	\begin{align*}
		\mu(P) + \mu(P \imp \At{K} \bot) & = \mu(P \lor (P \imp \At{K} \bot)) + 0 & & = \mu(P \lor (P \imp \At{K} \bot)), \\
		\mu(\At{K}P) + \mu(P \imp \At{K} \bot) & = \mu(\At{K} P \lor (P \imp \At{K} \bot)) + 0 & &  = 1.
	\end{align*}
	Thus upon hitting both equations with $\At{K}$, the claim follows from the cancellativity of \Cref{prop.at_order_canc} if we can prove that $\At{K} \mu(P \lor (P \imp \At{K} \bot)) = \At{K} 1$. For ease of notation, we abbreviate $Q := (P \lor (P \imp \At{K} \bot))$.

	Now we have, for every $q : \tQQ$,
	\begin{align*}
		\At{K} \mu(Q) & = \At{K} (\overline{ \At{K} \bot \lor \mu(Q) }) \\
			& = \At{K} \mu(\At{K} \bot \lor Q).
	\end{align*}
	where the second step is by \Cref{prop.valuations_OR}.
	\tob{continue here. doesn't quite work yet}
\end{proof}

\tob{We can probably get rid of everything from here to the appendix}
...
\begin{proof}
	...
	\]
	using the desired adjointness in the form that for all $R : \Op(X)$,
	\[
		\left[ R \land P \imp Q \right] \quad \Longleftrightarrow \quad \left[ R \imp (P \imp_{\Op(X)} Q) \right],
	\]
	together with the fact that the above $P \imp_{\Op(X)} Q$ is also open thanks to the assumption that arbitrary unions of opens are open.

	Now we have
	\begin{align*}
		\exists R : & \Op(X) \ldotp (R \land P \imp Q) \land R \\
