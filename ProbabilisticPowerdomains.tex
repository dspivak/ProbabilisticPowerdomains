...
\item \[\mu(P\wedge q) = \ol{P \wedge \mu(Q)}.\]
\end{enumerate}
\end{proposition} 
\begin{proof}
\begin{enumerate}
...
\item \todo{add this as per David's email from May 12}
\end{enumerate}
\end{proof}
...
\begin{proof}
The basic opens in $C\BaseSpace$ are given by the opens $U\subseteq \BaseSpace$, where the associated open in $C\BaseSpace$ contains all those compacts $K\subseteq \BaseSpace$ with $K\subseteq U$. We first show that the closure of this open consists of all compacts $K$ with $K\cap\overline{U} \neq \emptyset$. Clearly the complement of this set is open, since it is the open associated to the complement of $\overline{U}$. Moreover, it is the largest open with this property: if $V$ is such that $K\subseteq V$ implies $K \not\subseteq U$, then $V$ must be disjoint from $U$ since points are compact.
...
\begin{align*}
	...
	&\imp\At{K}\At{K}\mu(P)(q)&\text{\cite[Prop 5.39]{Schultz.Spivak:2017a}}\\	
	&=\At{K}\mu(P)(q).
\end{align*}
\end{proof}
