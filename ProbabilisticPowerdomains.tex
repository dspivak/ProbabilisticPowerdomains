...
\begin{definition}\label{def:flabby}
	...
	\forall (x,x' : X) \ldotp P(x) \wedge P(x') \imp x = x'
	\end{equation}
	holds. $X$ is \emph{flabby} if for every subsingleton $S\subseteq X$ there is $x\in X$ such that if $S$ is inhabited, then $x\in S$. In other words, $X$ is flabby if for every $P : X \to \Prop$,
