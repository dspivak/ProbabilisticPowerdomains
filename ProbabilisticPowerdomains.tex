...
\begin{lemma}
Let $j$ be any modality and $P : \Prop \to \Prop$ a predicate such that $U\imp V$ implies $P(V) \imp P(U)$. Then
\[
...
We prove $j P(V)$ for $V \coloneqq \left( \forall W \ldotp P(W) \imp W \right)$. Thus we can even assume $\exists U\ldotp P(U)$. By definition of $V$ and the assumption $P(U)$, we have $V \imp U$ for any such $U$, and therefore $P(U) \imp P(V)$ by assumption. Now the claim follows from $P(U)$.
\end{proof}
