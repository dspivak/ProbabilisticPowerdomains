...
\begin{proof}
	However, we start with the tautology
	\[
		\forall n : \tNN \ldotp \left( P(n) \imp \At{K} \bot \: \lor \: \exists n' : \tNN \ldotp P(n') \bot \right),
	\]
	...
	\]
	Now in case that there is $n$ with $\At{K} P(n)$, there is nothing more to prove. Hence we assume $P(n) \imp \At{K} \bot$ for all $n$, which is similar to what we did in the proof of \cref{lem.11_at_flabby}. But now together with $\At{K} \exists n : \ldotp P(n)$, we arrive at $\At{K} \bot$, which is enough.
\end{proof}
...
Suppose $X$ is an internal locale. A \emph{valuation} on $X$ is a function $\mu:\Op(X)\to\tLR$ satisfying the following:
\begin{enumerate}
	\item $\mu(\varnothing)=0$.
	\item $\forall(U,V:\Op(X))\ldotp(U\leq V)\imp\mu(U)\leq\mu(V)$.
	\item $\forall(U,V:\Op(X))\ldotp\mu(U)+\mu(V)=\mu(U\vee V)+\mu(U\wedge V)$.
\end{enumerate}
...
\footnote{Completely formally, $\mu(D)\coloneqq\{r:\RR\mid\exists(U:\subtype{D})\ldotp r=\mu(U)\}$.}
Let $\VV(X)$ denote the type of valuations $\mu$, ordered by $(\mu\leq\mu')\iff\forall(U:\Op(X))\ldotp(\mu U\leq \mu'U)$.
\end{definition}
