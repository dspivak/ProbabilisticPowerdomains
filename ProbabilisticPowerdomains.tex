...
\todo[inline]{Rewrite everything in here in the form of axioms together with soundness proofs?}
...
Given $Q$, we set $P \coloneqq \forall K \ldotp Q(K)$, and claim that $Q = \lambda K \ldotp \At{K} P$. This boils down to showing $Q(K) = \At{K} \forall K' \ldotp Q(K')$ for every $K$. Assuming $Q(K)$, we get $U : \Prop$ with $\At{K} U$ and $\At{K'} U \imp Q(K')$ for every $K'$. So in order to prove $\At{K} \forall K' \ldotp Q(K')$, we can even assume $U$, which implies $\At{K'} U$ and therefore $Q(K')$, as was to be shown.
\end{proof}
...
\begin{axiom}\label{ax:attonghbhd}
For every $P : \Prop$,
\[
...
\begin{proof}
The nontrivial direction is from left to right, so assume $\At{K} \exists x\ldotp P(x)$. Applying \cref{ax:attonghbhd} to $\exists x\ldotp P(x)$, we can assume $\exists U\ldotp \At{K} U \wedge \In{U} \exists x\ldotp P(x)$, since in the other case $\At{K}\bot$ we are done by inhabitedness of $X$, which follows from flabbiness. Thanks to \cref{prop:flabbiness_equivalence}, we even obtain $U$ such that $\At{K} U$ and $\exists x\ldotp \In{U} P(x)$. Choosing this $x$, we obtain $\At{K} P(x)$ from \cref{ax:atvsin}.
\end{proof}
...
It is convenient to identify $\smset$ with the category of $\At{B}$-sheaves in $\cat{E}$, and the external reals $\LR$ with $\tLR_{\At{B}}$. In other words, we identify external valuations with \emph{internal} maps
\[
	\mu' : \Prop_{\At{B}}^{\asSh_{\At{B}}(X)} \to \tLR_{\At{B}}
\]
which are probability valuations in the subtopos of $\At{B}$-sheaves.

So suppose that we start with a given $\mu$. Then we construct $\mu'$ as follows. The $P : \asSh_{\At{B}}(X) \to \Prop_{\At{B}}$ are in (internal) bijection with the $P' : X \to \Prop_{\At{B}}$ via composing with the sheafification map $\toSh : X \to \asSh_{\At{B}}(X)$, or equivalently the $\At{B}$-closed predicates on $X$. Under this identification, we put $\mu'(P) \coloneqq \At{B} \mu(P')$.
...
We now verify that these two constructions are inverse of one another. A preliminary observation is that the embedding $\Prop \to \Prop^{C\BaseSpace}$ commutes with $\See{B}$ thanks to \cref{lem:at_vs_seeb},
\[\begin{tikzcd}
	\Prop \ar{r} \ar{d}{\See{B}} & \Prop^{C\BaseSpace} \ar{d}{\See{B}} \\
	\Prop \ar{r} & \Prop^{C\BaseSpace}
\end{tikzcd}\]
Therefore when starting with $\mu'$, we can apply \cref{ax:enough_points}, so that we only need to prove that for all $\At{B}$-closed predicates $P$ and all $K$,
\[
	\At{K} \mu'(P)(q) = \See{B} \exists U \ldotp \At{K} U \wedge \mu'(\At{B} \In{U} P)(q).
\]
But since $P$ is $\At{B}$-closed, we have $\In{U} P = \In{U} \At{B} P = \At{B} P = P$, so that the right-hand side is just $\See{B} \mu'(P)(q)$. Since $\mu'(P)(q)$ is $\See{B}$-closed by assumption, it is enough to show that it is also $\At{K}$-closed, 

This is easy:
\begin{align*}
	\At{B} \left\{ K \:\Big|\: \sup_{U\ni K} \mu'(\At{B} \In{U} P)(q) \right\} & \\
	= & \sup_{U\ni B} \mu'(\At{B} \In{U} P)(q) \\
	= & \mu'(\At{B} \In{CB} P)(q) \\
	= & \mu'(\At{B} P)(q) = \mu'(P)(q).
\end{align*}
Conversely, suppose that we start with $\mu$. Then we need to prove that for any $P : X \to \Prop$ and $q\in \tQQ$,
\[
	\mu(P)(q) = \left\{ K \:\Big|\: \sup_{U\ni K} \At{B} \mu(\At{B}\In{U} P)(q) \right\}.
\]
or equivalently
\[
	\At{K} \mu(P)(q) = \sup_{U\ni K} \At{B} \mu(\At{B}\In{U} P)(q).
\]
\todo[inline]{This is not quite right yet, there's an $\At{B}$ missing on the left. Probably because the above construction of $\mu$ in terms of $\mu'$ isn't formulated properly yet}
which is exactly \cref{thm.discrete_internal_space}.
\end{proof}
