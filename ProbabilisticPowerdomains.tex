...
\todo[inline]{D: Do you mean for \cref{ax.atvsin} to be an iff?}
...
Denoting the closure of a set $U$ as $\ol{U}$ and the interior of a set $A$ as $A^\circ$, it is useful to note that 
\[\neg\neg U=\ol{U}^\circ.\]
...
Next, we show that the interior of the above closure is $U_{C\BaseSpace}=\{ K \mid K\subseteq U\}$ again. We need to show that it is not larger; so for a given $K$ in this interior, we need to prove $K\subseteq U$. Being in the interior of the above closure $\ol{U_{C\BaseSpace}}$ means that there is an open $V$ with $K\subseteq V$ such that every $K' \subseteq V$ is in the closure, which means $K' \cap \overline{U} \neq \emptyset$. Again since points are compact, we get $V \subseteq U$, and therefore $K \subseteq U$, as was to be shown.
...
For any $K\in CB$, we write $\At{K} := ((- \imp (\down K)^c) \imp (\down K)^c)$ for the associated quasi-closed modality, where $(\down K)^c$ is the (open) complement of $K$. Since $\down K$ is an irreducible closed set, it can be shown (along the lines of Lemma 5.37 of TTT) that we have:
...
\subsection{Compact $B$ case}

\begin{proposition}
Let $B$ be compact and $X\in\Shv(B)$ a sheaf. The set of external valuations on the set-theoretic frame $2^{X(B)}\in\smset$ is in natural bijection with the set of internal valuations on $\Omega^B$.
\end{proposition}
\begin{proof}
An internal valuation is a sheaf morphism $\mu\colon\Omega^X\to\tLR$. Here is the correspondence:
\begin{description}
  \item[Internal to external] Suppose that $\mu$ is an internal valuation, and let $S\ss X(B)$. Define
  \[\mu'(S)\coloneqq\mu_B(\At{B}S)(B).\]
  Here $\mu_B$ is the $B$-component of the natural transformation $\mu$, and $\At{B}S$ is the subsheaf of $X$ defined by
  \[
  \At{B}S(U)\coloneqq
  \begin{cases}
		X(U)&\tn{ if }B\not\ss U\\
  ...
  \end{cases}
  \]
  \item[External to internal] Given an external valuation $\mu'$, we define the corresponding internal valuation on every open $U$ as the lower real whose stalk at $K\ss U$ is
  \[\mu_U(T)(K)\coloneqq\sup_{V\supseteq K}\mu'\big(\rho_{B\to V}\inv(T(V))\big)
  \]
  for any $T\in\Sub(yU\times X)$. Note that the right-hand side does not depend on $U$.
\end{description}
We want to know that ``internal to internal'' is identity:
\[\mu_U(T)(K)=^?\sup_{V\supseteq K}\mu'\big(\rho_{B\to V}\inv(T(V))\big)(B)\]
for $K\ss U$. This is just \cref{thm.discrete_internal_space}.

External to external (should be easier).
\end{proof}
