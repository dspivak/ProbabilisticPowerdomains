...
\begin{proof}
	For every $K$ contained in an open $U$, we need to find a connected open neighborhood of $K$ contained in $U$. Since $U$ is open, there is $\hat{U} \subseteq \BaseSpace$ such that every $K'\subseteq \hat{U}$ is in $U$. Now the set of all compacts contained in $\hat{U}$ is a basic neighborhood of $K$ contained in $U$, and it is connected since its specialization preorder is connected.
\end{proof}
...
\begin{soundproof}
	We need to show that if $U \models \forall n\ldotp P(n) \lor \At{K} \bot$, then also $U \models \At{K} \forall n : \tNN \ldotp P(n)$. The assumption says equivalently that for all $V\subseteq U$ and $n\in \tNN(V)$, we have $V \subseteq P(n) \cup (\downarrow K)^c$. The claim is that if $K\in U$, then there is $W$ with $K\in W \subseteq U$ such that for all $W'\subseteq W$ and $n\in\tNN(W')$, we have $W' = P(n)$.

	So suppose $K\in U$, and consider the set
	\[
		W := \uparrow (U \cap (\downarrow K)).
	\]
	We show that $W$ is open, by proving that every $K'\in W$ has a neighborhood contained in $W$. By definition of $W$, there is $\tilde{K} \supseteq K'$ with $\tilde{K} \in U$ and $K \subseteq \tilde{K}$. By openness of $U$, we have an open $\tilde{U} \subseteq B$ such that $\tilde{K} \subseteq \tilde{U}$, and also every other $\hat{K} \subseteq \tilde{U}$ is in $U$. But then also $\hat{K} \in W$, since we have $ \hat{K} \subseteq (\hat{K} \cup \tilde{K}) \supseteq K$, and $\hat{K} \cup \tilde{K} \in U$. Hence $\tilde{U}$ is the desired neighborhood of $K'$ which lies in $W$.
	...
	\end{equation}
	We now show that $W$ is connected. So suppose that $W = W_1 \amalg W_2$, say with $K\in W_1$. Then every putative $K' \in W_2$ is such that there is $\tilde{K} \supseteq K$ with $\tilde{K} \supseteq K'$. Since these containments are specialization preorder relations, the first can hold only if $\tilde{K} \in W_1$, and the second one only if $\tilde{K} \in W_2$, which is a contradiction. Hence our assumption that there is $K' \in W_2$ was wrong, and we conclude $W_2 = \emptyset$, so that $W$ is connected.
...
Finally, our only axiom schema is the following:
...
\begin{proof}
	\tob{Needs to be adapted to the new definitions (hopefully it works out now)}
...
\begin{proposition}
	...
	In our intended semantics of sheaves no $\CB$, an inhabited sheaf $X$ is $\At{K}$-flabby if only if every germ at $K$ can be extended to a global section.
\end{proposition}
...
\begin{proof}
	...
	Let us write $1 + 1 = \{\alpha,\beta\}$ in terms of elements. Let $P : 1 + 1 \to \Prop$. In case $\At{K} P(\alpha)$, we use $x := \alpha$ and there is nothing more to prove; similarly $x := \beta$ if $\At{K} P(\beta)$. Hence we can assume $P(\alpha) \imp \At{K} \bot$ and $P(\beta) \imp \At{K} \bot$. But now we also have the additional assumption $\At{K}(P(\alpha) \lor P(\beta))$, which now implies $\At{K} \bot$, which is enough.
\end{proof}
