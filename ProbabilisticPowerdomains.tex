...
\begin{proof}
	...
	\end{align*}
	Using \cref{lem.at_plus_commute} we can hit both equations with $\At{K}$. Since $\At{K}(P \imp \At{K} \bot)\leq\At{K}1$, the claim will follow from the cancellativity of \Cref{prop.at_order_canc} if we can prove that $\At{K} \mu(P \lor (P \imp \At{K} \bot)) = \At{K} 1$. But this is now a consequence of \Cref{lem.mu_at_Q,lem.at_vs_or}.
\end{proof}
