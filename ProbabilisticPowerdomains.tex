...
\[\mu'(P)(q)\coloneqq \Gamma\left(\toSh_{\AtInline{B}}\left(\mu\left(\classify{\Sky_B(\top)}\circ\beta(P)\right)(q)\right)\right).\]
We still need to check that the $\mu'(P)$ is a lower real, and that $\mu'$ is an external valuation; we also have to produce an inverse.
\todo[inline, author=David]{Tobias, does this look right?}
