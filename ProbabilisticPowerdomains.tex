...
We next show that it is a normalized continuous valuation (NCV). Later we will show that, under some conditions, it is unique such.
...
\item Using inclusion-exclusion, \cref{prop.unique_NCV_on_prop}, and the fact that $\min(a,b)+\max(a,b)=a+b$ for any lower reals, $a,b:\tLR$, we have
\begin{align*}
...
If $\BaseSpace$ is Hausdorff, then the closure of a point $K\in C\BaseSpace$ is $\downarrow K$, since any $K'\not\supseteq K$ can be separated from a chosen $b\in K\setminus K'$ by an open. It follows that the specialization preorder is reverse containment\footnote{The Sierpinski space shows that this is not true in general.}, and it is directed complete~\cite[Proposition~3.1]{Edalat:1995a}.
...
\todo[inline]{Is it necessarily a copy of Set?}
...
Since $\At{K}\bot$ is a constant predicate, we have $\mu(\At{K}\bot)=i(\At{K}\bot)=\ol{\At{K}0}$, by \cref{prop.unique_NCV_on_prop}. Therefore we have 
\begin{align*}
	\ol{\At{K}0}+1
...
\begin{align*}
	...
	&=\At{K}\In{U}\mu(P)(q)&\text{\cref{prop.In_NCV}}\\
...
In the interval domain, the following proposition applies for $U=\upclose[d',u']$ and $K=[d,u]$ in case $d'\leq u$ and $d\leq u'$, i.e.\ in case the intervals $[d,u]$ and $[d',u']$ have nontrivial intersection. In that case $\In{U}\bot\imp\See{K}\bot$.

\begin{proposition}
Let $P$ be a decidable predicate, and suppose $U$ is coprime. If $U^c\ss\down K$, then
\[\See{K}\mu(P)=\See{K}\mu(\In{U}P)\]
and similarly, $\At{K}\mu(P)=\At{K}\mu(\In{U}P)$.
\end{proposition}
\begin{proof}
We begin with the $\See{K}$ case. By assumption, we have $\In{U}\bot\imp\See{K}\bot$.

For any $P$ we have $\See{K}\mu(P)\leq \See{K}\mu(\In{U}P)$, and a similar inequality for $\neg P$. The following shows that the sum of these inequalities is an equality, completing the proof in the $\See{}$ case:
\begin{align*}
	\See{K}(1)
	&=\See{K}\mu(P\vee\neg P)&P\text{ decidable}\\
	&=\See{K}\mu(P)+\See{K}\mu(\neg P)&\text{incl-excl}\\
	&\leq\See{K}\mu(\In{U}P)+\See{K}\mu(\In{U}\neg P)&\text{modality}\\
	&=\See{K}\mu(\In{U}\bot)+\See{K}\mu(\In{U}P\vee\In{U}\neg P)&\text{incl-excl}\\
	&=\See{K}\mu(\In{U}\bot)+\See{K}(1)&\text{$U$ is coprime}\\
	&\leq\See{K}\mu(\See{K}\bot)+\See{K}(1)&\In{U}\bot\imp\See{K}\bot\\
	&=\See{K}\mu(\bot)+\See{K}(1)=\See{K}(1)&\text{\cref{thm.logical_reformulation}}
\end{align*}
Since $\See{K}\bot=\At{K}\bot$, we again have $\In{U}\bot\imp\At{K}\bot$. We repeat the above reasoning---mutatis mutandis---for the $\At{K}$-case, i.e.\ simply replacing $\See{}$ with $\At{}$ everywhere.
\end{proof}
...
While the results of the previous sections were completely internal, we now use them in order to analyze the semantics of NCV's in $\Shv(C\BaseSpace)$.
...
that is Scott continuous (at least in each variable) and an NCV for each point $a\in\pt(\cat{E})$.
...
\nonumber
	...
	&=\sup_{U\ni K}\In{U}\mu(P)(K)&\text{\cref{prop.In_NCV}}\\\nonumber
	&=\mu(P)(K)&\text{item 3}
\end{align}
\end{proof}
...
Here, we will investigate how NCV's on spaces of sections can be identified with stochastic processes in the conventional sense.

\chapter{Open questions}

Do NCV's internal to some topos $\cat{E}$ give rise to a probability monad?
