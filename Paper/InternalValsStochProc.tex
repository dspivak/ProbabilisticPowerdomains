...
The type of $\At$-closed propositions has the following semantics.
...
\begin{proof}
	Upon noting that $\At \prop^X$ is isomorphic to the function type $X \to \At \prop$, this follows from the previous lemma and a straightforward application of the standard semantics of function types.
\end{proof}
...
Using Moerdijk's \Cref{moerdijks_theorem}, the lemma shows that the sheaf of sections of $p$ comes with a canonical internal topology in $\shv(B)$. We denote the resulting internal space by $(E_p, \Open(E_p))$.

\begin{lemma}
	\label{opens_semantics}
	We have, for open $U \subseteq B$,
	\[
		\church{\Open(E_p)}(U) \cong \{ \textrm{open sets in } \germs{E}{} \cap p^{-1}(U) \},
	\]
