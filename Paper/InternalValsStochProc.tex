...
\footnote{Here $r_>$ is just a symbol, but it reminds the reader that a lower real $r$ is defined as the set of nonnegative rationals that are lower bounds for $r$.}
\]
We often write $r>q$ or $q<r$ to mean the proposition $r_>(q)$. We often refer to lower nonnegative reals simply as \emph{lower reals}.
\end{definition}
