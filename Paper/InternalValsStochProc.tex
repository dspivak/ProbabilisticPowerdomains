...
We have assumed that $T$ is locally connected, meaning that for every point $\pt\in\Pts$ and open neighborhood $U\ni\pt$, there exists a connected open neighborhood $U'\ni\pt$ with $U'\ss U$. This is what we need for the soundness of \cref{ax.N_flabby}.
...
\begin{soundproof}
Suppose $T\forces\At (\exists n : \tnn) . P(n)$; then there is a neighborhood $U\ni\pt$ such that $U\forces(\exists n : \tnn) . P(n)$, so take $n\in\tnn(U)$. By local connectedness, we may assume $U$ is connected, in which case $n=\rest{n'}{U}$ is the restriction of some global section $n'\in\tnn(T)$, proving the result.
\end{soundproof}
...
\begin{definition}\label{def.valuations_compatible_lowersemi}
	...
	\begin{enumerate}
		\item \emph{compatible} if for all $\pt, \pt' : \Pts$ and $P : \At \Op[X]$,
			...
			\]
			\dis{This is the same as $\At[\pt'] \mu_{\pt'}(P) = \At[\pt'] \mu_\pt(P)$, correct? Somehow I find this expression easier to think about because $P$ is not changing. It's more symmetric between $k$ and $k'$.}
