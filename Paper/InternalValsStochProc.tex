...
\begin{proof}
	First of all, \eqref{mu_stitched} is indeed a lower real thanks to \Cref{prop:LR_locality}. For verifying the conditions required of a valuation, it is enough to prove that they hold $\At$-locally, thanks to the enough points condition. To do so, we first prove that for every $P : \Op_X$,
	\begin{equation}
	...
	\begin{equation}
		...
		\mu_\pt(\At \exists P : \Op_X . D(P) \land P) \: \le \: \At \exists P : \Op_X . D(P) \land \mu_k(\At P),
	\end{equation}
	...
	\]
	We claim that $D'$ is $\At$-locally directed.
	\tob{under construction}
\end{proof}
...
\begin{theorem}
	\label{main_external}
	Let $p : E \to B$ be a continuous map between topological spaces. Then there is a bijection between:
	\begin{enumerate}
		\item Internal valuations in $\shv(B)$ on the internal space associated to $p$.
		\item ???
			\tob{Something like: families of valuations on the stalks of sections of $p$, subject to suitable conditions}
	\end{enumerate}
