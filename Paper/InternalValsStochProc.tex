...
\begin{enumerate}[resume]
	...
	\[\sup_{U\in D}\mu(U)=\mu\big(\exists(U:D).U\big).\]
\end{enumerate}
...
\begin{proof}
	...
	which means that $P \lor Q$ and $P \land Q$ both hold.\dis{there are other cases to consider, e.g.\ $P\wedge Q=\bot$} But then clearly also $P$ and $Q$ hold, so that $q_\lor < \ind{P}$ and $q_\land < \ind{Q}$, which proves the desired $q < \ind{P} + \ind{Q}$. For the other direction, suppose that $q < \ind{P} + \ind{Q}$. This means that we have $q_P$ and $q_Q$ with $q = q_P + q_Q$ as well as $q_P < \ind{P}$ and $q_Q < \ind{Q}$. But then both $P$ and $Q$ hold, and therefore so do $P \lor Q$ and $P \land Q$. This proves that $q_P < \ind{P \lor Q}$ and $q_Q < \ind{P \land Q}$.
	...
	\[
		\ind{\exists P : \prop . D(P) \land P} = \sup_{P : \prop \:\mid\: D(p)} \ind{P},
	\]
	again by proving both inequalities. If $q < \ind{\exists P : \prop . D(P) \land P }$, then $\exists P : \prop . D(P) \land P$ holds. But then $q < \ind{P}$ as well, from which the claim follows. For the other inequality, if $q < \sup_{P : \prop \:\mid\: D(p)} \ind{P}$, then there must be $P : \prop$ such that $D(P)$ and $q < \ind{P}$, so that also $P$ holds. But then we have $\exists P : \prop . D(P) \land P$, and hence also $q < \ind{\exists P : \prop . D(P) \land P}$. This completes the first half, showing that $\inc : \prop \to [0,1]$ is indeed a valuation.
\end{proof}
...
\end{proposition}
\begin{proof}
By \cref{vals_on1} it suffices to show that an arbitrary valuation $\mu : \prop \to [0,1]$ on $1$ satisfies $\mu(P) = \ind{P}$ for all $P : \prop$. One direction is easy, namely
	\[
		\ind{P} \le \mu(P).
	\]
	This follows because $q < \ind{P}$ for $q > 0$ implies $P$, and therefore $\mu(P) = \mu(\top) = 1$. The other inequality direction takes some more work. For fixed $P$, consider the predicate
	\[
		D(Q) := \left( \mu(Q) = \ind{Q} \right) \land (Q \Rightarrow P).
	\]
	We claim that $D$ defines a directed subtype of $\prop$. Indeed $D$ is inhabited by $\bot$. Now suppose that $D(Q_1)$ and $D(Q_2)$ for $Q_1, Q_2 : \prop$. We then claim that also $D(Q_1 \lor Q_2)$; since $Q_1 \Rightarrow P$ and $Q_2 \Rightarrow P$, the part $(Q_1 \lor Q_2) \Rightarrow P$ is clear, so the main problem is to show that also $\mu(Q_1 \lor Q_2) = \ind{Q_1 \lor Q_2}$. Using the assumed modularity equation, we have
	\begin{align*}
		\inc{Q_1 \lor Q_2} + \ind{Q_1 \land Q_2}	& \le \mu(Q_1 \lor Q_2) + \mu(Q_1 \land Q_2)	\\
								& = \mu(Q_1) + \mu(Q_2) \\
								& = \ind{Q_1} + \ind{Q_2},
	\end{align*}
	...
	implies that the inequality is actually an equality. Since the expressions are ordered pointwise, $\ind{Q_1 \lor Q_2} \le \mu(Q_1 \lor Q_2)$ and $\ind{Q_1 \land Q_2} \le \mu(Q_1 \land Q_2)$, the equality of their sum implies that both of these inequalities are actually equalities. Overall, we have therefore shown that $D(Q_1 \lor Q_2)$, making $D$ directed.
	...
	\]
	But now since $D(Q)$ implies $\mu(Q) = \ind{Q}$ by definition, the right-hand side further evaluates to
	\[
		\mu(P) = \sup_{Q : \prop \: \mid \: D(Q)} \ind{Q} = \ind{P},
	\]
...
\begin{corollary}
	Let $(X,\Op_X)$ be a topological space and $\mu : \Op_X \to [0,1]$ any valuation. Then $\mu(P) = \ind{P}$ for all $P : \prop$.
\end{corollary}
