...
\footnote{Here $r_>$ is just a symbol, but it reminds the reader that a lower real is defined as the set of nonnegative rationals that are lower bounds.}
\begin{enumerate}
...
\noindent(Down-closed): Choose rationals $q'>q$ with $r_1+r_2>q'$. By definition we have $q_1',q_2'$ with $r_1>q_1'$ and $r_2>q_2'$ and $q'=q_1'+q_2'$. Let $\epsilon\coloneqq (q'-q)/2$ and for each $i=1,2$, let $q_i\coloneqq q_i'-\epsilon$. Then $r_1>q_1$ and $r_2>q_2$ and $q=q_1+q_2$, so $r_1+r_2>q$ as desired.
...
\begin{proposition}[Order cancellativity]
Suppose $r_0,r_1,r_2:\trr$ are lower reals and that $r_0$ is bounded in the sense that $q_0\geq r_0$ for some $q_0:\tqq$. If $r_0+r_1\geq r_0+r_2$ then $r_1\geq r_2$.
\end{proposition}
\begin{proof}

\end{proof}

\begin{definition}[Propositions ``are'' lower reals]\label{def.props_are_reals}
There is a function $\prop\to\tlrr$ sending $p:\prop$ to the function $\tqq\to\prop$ given by $q\mapsto(q<0)\vee p$.%
\footnote{We leave it to the reader to check that this is indeed a lower nonnegative real.}
\end{definition}

\begin{proposition}
Let $p,p':\prop$ be propositions, considered as lower reals. Then for any $r:\trr$ we have 
\begin{enumerate}
	\item $p'\leq p$ iff $p'\imp p$.
	\item $p+r=p\vee r$.
\end{enumerate}
\end{proposition}
\begin{proof}
If $p'\leq p$ then applying \eqref{eqn.real_order} with $q=1$ we have $p'\imp p$ as desired. The converse is similarly straightforward. The rest of the proof deals with the second claim; recall that $(p+r)>q$ means $\exists(q_p,q_r:\tqq).(0>q_p\vee p)\wedge(r>q_r)\wedge (q=q_p+q_r)$.

We first show that $p\vee (r>q)$ implies $(p+r)>q$. If $p$ then taking $q_r=-1$ and $q_p=1+q$, we have $(p+r)>q$. If $r>q$ then by roundedness we can find $q_r:\tqq$ such $r>q_r>q$. Then with $q_p\coloneqq q-q_r$ we have $(p+r)>q$.

Finally, we show $(p+r)>q$ implies $p\vee(r>q)$. Assuming $(p+r)>q$, we have either $p$ or $q_p,q_r:\tqq$ with $0>q_p$ and $r>q_r$ and $q=q_p+q_r$. In the first case we're done, and in the second, $r>q_r>q_r+q_p=q$.
\end{proof}
