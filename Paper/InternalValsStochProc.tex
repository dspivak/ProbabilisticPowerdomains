...
\newcommand{\tnnqq}{{\internal{Q}\hs}_{\geq0}}
\newcommand{\tQQp}{\tQQ_{+}}
...
Let $\tnnqq$ denote the nonnegative rationals. We define the type of \emph{lower nonnegative reals}, denoted $\tlrr$ as
\[
\tlrr=\{r_>\colon\tnnqq\to\prop\mid\forall(q:\tqq). r_>q\iff\exists(q':\tqq).r_>q'\wedge q'>q\}.%
...
It is often convenient to have names for the two directions of the $\iff$. The implication $r_>q\imp\exists(q':\tqq).r_>q'\wedge q'>q$ is called \emph{roundedness} and its inverse is called \emph{down-closure}.
\end{definition}
...
%\[
%	r>r'\coloneqq\exists (q:\tnnqq).r>q\wedge\forall(q'.\tnnqq).r'>q'\imp q>q'.
%\]
we say that $r$ \emph{is above} $r'$ and write $r\geq r'$ to denote the following proposition:
\[
	r\geq r'\coloneqq\forall(q:\tnnqq).r'>q\imp r>q.
\]
\end{definition}
...
Let $r_1,r_2:\tlrr$ be lower reals. Define their \emph{sum}, denoted $(r_1+r_2)_>:\tnnqq\to\prop$ to be the predicate%
\footnote{The first two clauses, $(r_1=\bot\wedge r_2>q)\vee(r_2=\bot\wedge r_1>q)$, are the cost of only testing on $\tnnqq$ rather than on all of $\tqq$. Without these clauses, $0$ is not a unit for addition.}
\begin{multline}\label{eqn.sum_lr}
(r_1+r_2)_>q\coloneqq
(r_1=\bot\wedge r_2>q)\vee(r_2=\bot\wedge r_1>q)\vee\\\exists (q_1,q_2:\tnnqq).(r_1>q_1)\wedge (r_2> q_2)\wedge (q=q_1+q_2).\end{multline}
...
\end{proposition}
\begin{proof}
We need to show that $\forall(q:\tqq). (r_1+r_2)_>q\iff\exists(q':\tqq).(r_1+r_2)_>q'\wedge q'>q$.

\noindent($\imp$): For roundedness, choose a rational $q$ and suppose $(r_1+r_2)>q$. By definition we have $q_1,q_2$ with $r_1>q_1$ and $r_2>q_2$ and $q=q_1+q_2$. By roundedness of $r_1$ and $r_2$, we have rationals $q_1',q_2'$ with $r_1>q_1'>q_1$ and $r_2>q_2'>q_2$. Then letting $q'=q_1'+q_2'$, we have $(r_1+r_2)>q'$ and $q'>q$, as desired.

\noindent($\Leftarrow$): For down-closure, choose rationals $q'>q$ with $r_1+r_2>q'$. By definition we have $q_1',q_2'$ with $r_1>q_1'$ and $r_2>q_2'$ and $q'=q_1'+q_2'$. Let $\epsilon\coloneqq (q'-q)/2$ and for each $i=1,2$, let $q_i\coloneqq q_i'-\epsilon$. Then $r_1>q_1$ and $r_2>q_2$ and $q=q_1+q_2$, so $r_1+r_2>q$ as desired.
\end{proof}

\begin{definition}[Rationals ``are'' lower reals]
There is a function $\tnnqq\to\tlrr$ sending $q$ to the predicate $q_>\colon\tnnqq\to\prop$ given by $q_>q'\coloneqq q>q'$.%
\footnote{We leave it to the reader to check that $q_>$ is indeed a lower real.}
 We often write $q:\tlrr$ to denote this lower real. In particular $0$ denotes the lower real $q\mapsto\false$.
\end{definition}
...
\end{proposition}
\begin{proof}
Commutativity and unitality of $0$ are obvious from the definition \eqref{eqn.sum_lr}. It remains to test associativity. but now there are nine cases!!
\end{proof}
