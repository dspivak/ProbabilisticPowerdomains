...
The sets of local sections of $p$ form a sheaf on $B$. Its \'etalification $\germs{E}{}$ is another space consisting of all the germs of local sections of $p$; we have the usual local homeomorphism $\germs{E}{} \to B$, and moreover $\germs{E}{}$ carries another topology generated by those sets of germs which have representatives contained in a given open of $E$.

\begin{lemma}
	This topology is (non-strictly) coarser than the \'etale topology, and such that the projection $\germs{E}{} \to B$ is continuous.
\end{lemma}

\begin{proof}
	For the first claim, we need to show that the set of germs contained in a given open $U \subseteq E$ is also \'etale open. So for a given germ $(x,[f])$ in $U$ represented by $x \in B$ and a local section $f : V \to E$, we need to find an \'etale neighborhood which is contained in the given open specified by $U$.

	The assumption that the germ $(x,[f])$ is in $U$ means that we can assume $f(V) \subseteq U$ by choosing $V$ small enough. But then the germ of $f$ at any point in $V$ lands in $U$, and this defines the desired \'etale neighborhood.

	For continuity of the projection $\germs{E}{} \to B$, consider an open $V \subseteq B$. Then its preimage consists of all germs defined on subopens of $V$. All of those have range contained in $p^{-1}(V)$, and therefore form an open set in $\germs{E}{}$.
\end{proof}

Using Moerdijk's \Cref{moerdijks_theorem}, the lemma shows that the sheaf of sections of $p$ comes with a canonical internal topology in $\shv(B)$.
