...
\dis{This uses commutation of $j$ with addition of lower reals, which follows from ``axiom 4''.}
	...
	\begin{itemize}
		\item If $Q$ is any $j$-closed open, then $\mu(Q)$ is a $j$-closed lower real.
			...
			\begin{align}
				...
				\end{split}
			\end{align}
			When applied to any rational $q$, the right-hand side is clearly $j$-closed, hence so is the left-hand side. But by \cref{cor.closed_sum}, a sum of lower reals is $j$-closed if and only if each summand is, and it follows that $\mu(Q) = j \mu(Q)$, as was to be shown.
	...
	\]
	and it follows that $j \mu(P \land (P \Rightarrow j\bot)) =  j \ind{\bot}$. Finally, we have $j \mu(P \lor (P \Rightarrow j\bot)) = j 1$ by the second item above, since $j(P \lor (P \Rightarrow j\bot))$ clearly holds if $jP$ and also if $P \Rightarrow j\bot$. Overall, we therefore get
	\[
