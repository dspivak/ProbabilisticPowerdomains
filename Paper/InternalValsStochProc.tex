...
\begin{enumerate}[label=4'.]
	\item $\Open_X(\exists(P:\Op_X).S(P)\wedge P)$ for any $S\colon\Op_X\to\prop$. 
\end{enumerate}
\begin{enumerate}[label=4''.]
	\item $\Open_X(\exists(P:S).P)$ for any $S\ss\Op_X$.
\end{enumerate}
...
Recall from \Cref{space_localize} that the object of $j$-closed opens, $j \Op_X$, is a $j$-local internal space.
...
\begin{proof}
	...
	\]
	The predicate $\exists P : j \Op_X . D(P) \land P$ is again open, and we can thus apply \Cref{vals_vs_modalities} to remove the inner $j$ on the left-hand side. Furthermore replacing the existential quantification by an equivalent one, it is hence enough to prove
	\[
		j \mu\left( \exists Q : \Op_X . D'(Q) \land Q \right) = j \exists Q : \Op_X . D'(Q) \land j \mu(Q).
	\]
	with the predicate
	\[
		D' \: : \: \Op_X \longrightarrow \prop, \qquad Q \longmapsto D(j Q) \land (j Q \imp Q).
	\]
	If $D'$ is directed, then Scott continuity of $\mu$ gives
	\[
		\mu\left( \exists Q : \Op_X . D'(Q) \land Q \right) = \exists Q : \Op_X . D'(Q) \land \mu(Q).	
	\]
	Since every $Q$ with $D'(Q)$ is $j$-closed, we may as well use $j \mu(Q)$ on the right-hand side by \Cref{vals_vs_modalities}. This proves the claim by an application of $j$. It thus remains to be argued that $D'$ is directed.

	Inhabitedness of $D'$ clearly follows from inhabitedness of $D$. If $D'(Q_1)$ and $D'(Q_2)$, then in particular both $Q_1$ and $Q_2$ are $j$-closed and satisfy $D(Q_1)$ and $D(Q_2)$. Therefore by directedness of $D$ there is $j$-closed $Q$ with $Q_1 \imp Q$ and $Q_2 \imp Q$ as well as $D(Q)$. But then also $D'(Q)$, as was to be shown.
\end{proof}
...
\begin{axiom}[{$\At[]$-Local decidability}]
...
\begin{axiom}[Enough points]
	For all $P : \prop$,
	\[
		\left[ \forall \pt : \Pts . \At P \right] \: \Rightarrow \: P.
	\]
\end{axiom}
...
\begin{proof}
	For every $r : \tlrr$, the family with $r_k\coloneqq \At r$ for each $k\in\Pts$ satisfies lower semicontinuity; indeed for any $q$ we can take $U\coloneqq r(q)$ and \eqref{eqn.witness} holds by definition.
...
\begin{definition}
	A family of $\At[]$-local valuations $(\mu_\pt)_{\pt : \Pts}$ is \emph{compatible} if for all $\pt, \pt' : \Pts$ and $P : \At \Op_X$,
	\[
...
Recall from \Cref{val_localize} that if $\mu : \Op_X \to \tlrr$ is a valuation, then for every $\pt$ the restriction of $\mu$ to $\At \Op_X$ is an $\At$-local valuation. 

\begin{lemma}
	Let $\mu : \Op_X \to \tlrr$ be a valuation. Then the family $\left( \mu|_{\At \Op_X} \right)_{\pt : \Pts}$ is a compatible family of $\At$-local valuations.
\end{lemma}

\begin{proof}
	We need to show that if $P : \At \Op_X$, then
	\[
		\mu( \At[\pt'] P ) = \At[\pt'] \mu(P).
	\]
	But this is exactly \Cref{vals_vs_modalities}.
	\tob{Or rather the special case of \Cref{vals_vs_modalities} where $P$ is $\At$-closed. This may be relevant if we try to eliminate appeal to \Cref{ax:N_flabby}}
\end{proof}
