...
\begin{proposition}
Let $p,p':\prop$ be propositions, considered as lower reals. Then 
\begin{enumerate}
...
\end{proposition}
\begin{proof}
If $p'\leq p$ then applying \eqref{eqn.real_order} with $q=1$ we have $p'\imp p$ as desired. The converse is similarly straightforward.

Suppose $(p+p')>q_0$, and let $q,q'$ be such that $p>q$, $p'>q'$, and $q_0=q+q'$. If $0>q_0$ then either $0>q$ or $0>q'$, so either $p$ or $p'$. Rationals are decidable so either $(0>q_0)\vee\neg(0>q_0)$ holds, so we may assume $\neg(0>q_0)$. Then either $0>q$ or $0>q'$, so either $p$ or $p'$ holds by definition.
\end{proof}
