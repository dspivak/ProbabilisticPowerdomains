...
\begin{lemma}
	Let $\Op[X]$ be a topology on any set $X$. Then for any $U,V \in \Op[X]$, we have $(U \imp V) \in \Op[X]$.
\end{lemma}

\begin{proof}
	The Heyting implication $U \imp V$ is the union of all $W \subseteq X$ for which $U \cap W \subseteq V$.
	\tob{continue here}
\end{proof}
...
\begin{theorem}
	...
	Let $j$ be a decidable modality and $\mu \colon \Op[X] \to \tlrr$ a valuation on a topological space $X$. Then for all $U \in \Op[X]$,
	\[
		\mu(jU) = j \mu(U).	
	\]
\end{theorem}

Note that if $U$ is open then so is $jU$ by \Cref{props_open}, and therefore indeed in the domain of $\mu$ to begin with.

\begin{proof}
	We first prove two auxiliary statements, both of which are special cases of the overall result.
	\begin{itemize}
		\item If $V$ is any $j$-closed open, then $\mu(V) \in \tlrr$ is a $j$-closed lower real.
			
			To begin the proof, we have the equivalences
			\begin{align*}
				V \cap (Q \Rightarrow j\bot) \quad & \Longleftrightarrow \quad j\bot, \\
				V \cup (Q \Rightarrow j\bot) \quad & \Longleftrightarrow \quad \top,
			\end{align*}
