...
\newcommand{\inc}{i}				% inclusion of Prop in lower reals
...
\begin{definition}[Lower nonnegative reals]
...
There is a function $\prop\to\tlrr$ sending $p:\prop$ to the function $1_p\colon\tqq\to\prop$ given by:%
\footnote{We leave it to the reader to check that $1_p$ is indeed a lower nonnegative real.}
\[1_p(q)\coloneqq(q<0)\vee ((q<1)\wedge p).\]
\end{definition}
...
We want to show order cancellativity, \cref{prop.order_cancel}. To do so we follow a proof of Simon Henry \cite{Henry}.%\url{https://mathoverflow.net/questions/108029/simplification-in-semi-continuous-real?rq=1}
\begin{lemma}
For any $p:\prop$ and $r_1,r_2:\tlrr$, if $1_p+r_1\leq 1_p+r_2$ then $r_1\leq r_2$.
\end{lemma}
\begin{proof}
Suppose $q_1<r_1$. Then by \cref{lemma.sums_bigger}, $q_1<1_p+r_1$ so by assumption $q_1<1_p+r_2$. By \cref{def.sum_lr}, there exist $q<1_p$ and $q_2<r_2$ with $q_1=q+q_2$. There are two cases: $q<0$ or $(q<1)\wedge p$. If $q<0$ then $q_1=q+q_2<q_2<r_2$, as desired. In the second case, we have $p$ so by \cref{eqn.indicator,prop.cancel_rationals}, $1_p+r_1\leq 1_p+r_2$ directly implies $r_1\leq r_2$.
\end{proof}

\begin{proposition}[Order cancellativity, \cite{Henry}]\label{prop.order_cancel}
Suppose $r,r_1,r_2:\trr$ are lower reals and that $r$ is bounded in the sense that $r\leq N$ for some $N:\tnn$. If $r+r_1\leq r+r_2$ then $r_1\leq r_2$.
\end{proposition}
\begin{proof}
For positive integer $n$, define $h_n:\tlrr$ to be the following linear combination of indicator functions:
\[
h_n\coloneqq \frac{1}{n}\cdot\sum_{i=1}^{n\cdot N}1_{(i/n)<r}
\]
We first claim that $h_n\leq r$, so take $q<h_n$; we will show $q<r$. By \cref{def.Q_action,def.sum_lr}, we have $nq=q_1+\cdots+q_{nN}$ with $q_i<0$ or $(q_i<1)\wedge (i/n<r)$ for each $i$. Let $i_r$ be the largest $i$ with $0<q_i$. Then $i_r/n<r$ and hence for all $i\leq i_r$ we have $i/n<r$, so $q_i<1$, and for all $i>i_r$ we have $q_i<0$. It follows that $nq<i_r$, i.e.\ that $q<i_r/n<r$, as desired.

We next claim that $r\leq h_n+1$, so take $q<r$; we will show $q<h_n+1$. Let $n_0\coloneqq\floor{nq}$ be the largest integer with $n_0\leq nq$.**Finish**
\end{proof}
...
\end{corollary}
\begin{proof}
If $r_1\leq r_1'$ then $r_1+r_2'\leq r_1'+r_2'=r_1+r_2$, so $r_2'\leq r_2$ by \cref{prop.order_cancel}. The second assumption $r_2\leq r_2'$ implies $r_2=r_2'$, and similarly we find $r_1=r_1'$.
\end{proof}

% ==== Section ====%
\section{Internal topological spaces and Moerdijk's theorem}

If $(X,\Op_X)$ is a topological space, then the fact that $\Op_X$ is closed under joins means the following: if $S : \Op_X \to \prop$ specifies any subtype of $\Op_X$, then also its join
\[
	\exists P : \Op_X . S(P) \land P
\]
is open, i.e.~satisfies the predicate $\Op_X : (X \to \prop) \to \prop$. This is a powerful assumption with the following surprising consequences.

\begin{lemma}
	\label{props_open}
	Let $(X,\Op_X)$ be a topological space. Then:
	\begin{enumerate}
		\item For every open $P : \Op_X$ and every modality $j$, also $jP$ is open.
...
\begin{proof}
	\begin{enumerate}
		\item Consider the join of the predicate $S : \Op_X \to \prop$ given by
			\[
				S(Q) \: := \: j(Q \Rightarrow P).
			\]
			This join is
			\[
				\exists Q : \Op_X . j(Q \Rightarrow P) \land Q,
			\]
			which is easily seen to be equivalent to $j P$ (by taking $Q := \top$ in the reverse direction). Hence $jP$ is indeed open.
		\item Apply the previous statement to the open $\bot$ and the closed modality $P \lor -$. \qedhere
	\end{enumerate}
\end{proof}
...
\begin{enumerate}[resume]
	\item for all directed sets $(U_i)_{i\in I}\ss\Op_X$,%
...
Recall that $\inc : \prop \to [0,1]$ is the inclusion of propositions into the unit interval, given by $\inc(P)(q) := P$.
...
\begin{proof}
	We first show that $\inc$ is indeed valuation. The normalization conditions are trivial, while monotonicity is as follows: if $P \Rightarrow Q$, then also $\inc(P) \le \inc(Q)$, since clearly
	\[
		\inc(P)(q) \: \Longrightarrow \: P \: \Longrightarrow \: Q \: \Longrightarrow \: \inc(Q)(q)
	\]
	for all $q$. Concerning modularity, for $P,Q : \prop$ we prove both inequality directions of the claimed equation
	\[
		\inc(P \lor Q) + \inc(P \land Q) = \inc(P) + \inc(Q).
	\]
	Indeed $q < \inc(P \lor Q) + \inc(P \land Q)$ means that there are $q_\lor$ and $q_\land$ such that $q = q_\lor + q_\land$ and
	\[
		q_\lor < \inc(P \lor Q), \qquad q_\land < \inc(P \land Q),
	\]
	which means that $P \lor Q$ and $P \land Q$ both hold. But then clearly also $P$ and $Q$ hold, so that $q_\lor < \inc(P)$ and $q_\land < \inc(Q)$, which proves the desired $q < \inc(P) + \inc(Q)$. For the other direction, suppose that $q < \inc(P) + \inc(Q)$. This means that we have $q_P$ and $q_Q$ with $q = q_P + q_Q$ as well as $q_P < \inc(P)$ and $q_Q < \inc(Q)$. But then both $P$ and $Q$ hold, and therefore so do $P \lor Q$ and $P \land Q$. This proves that $q_P < \inc(P \lor Q)$ and $q_Q < \inc(P \land Q)$.
	...
	Now let $\mu : \prop \to [0,1]$ be any valuation on $1$. We will then want to prove that $\mu(P) = \inc(P)$ for all $P : \prop$. One direction is easy, namely
	\[
		\inc(P) \le \mu(P).
	\]
