...
\begin{corollary}
	\label{val_localize}
	Let $j$ be a decidable modality and $\mu : \Op_X \to \tlrr$ a valuation. Then
	\[
		j \Op_X \longrightarrow j \tlrr, \qquad P \longmapsto j \mu(P)
	\]
	produces a $j$-local valuations.
\end{corollary}

\begin{proof}
	The normalization conditions and monotonicity are obvious. The modularity equation holds because $j$ commutes with addition of lower reals.
	\tob{Again assuming that $j$ commutes with $\exists q : \tqq$}
