...
\begin{corollary}
Suppose $j\exists(q:\tqq).P(q)\imp \exists(q:\tqq).jP(q)$ holds for every $P:\tqq\to\prop$. Then for any $r_1,r_2:\tlrr$, if $r_1+r_2$ is $j$-closed then so is $r_1$.
\end{corollary}
\begin{proof}
Clearly $r_1\leq jr_1$ and $r_2\leq jr_2$. By \cref{prop.commute_exists_commute_addition}, if $r_1+r_2=j(r_1+r_2)$ then $r_1+r_2=jr_1+jr_2$, and the result follows from \cref{cor.favorite}.
\end{proof}
	...
	\begin{itemize}
		\item If $Q$ is any $j$-closed open, then $\mu(Q)$ is a $j$-closed lower real.
			...
			\begin{align}
				...
				\end{split}
			\end{align}
			When applied to any rational $q$, the right-hand side is clearly $j$-closed, hence so is the left-hand side. But since a sum of lower reals is $j$-closed if and only if each summand is,\dis{Why does $a+b$ $j$-closed imply $a$ is $j$-closed?} it follows that $\mu(Q) = j \mu(Q)$, as was to be shown.
