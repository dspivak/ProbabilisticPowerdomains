...
\begin{proof}
	...
	\begin{itemize}
		\item If $Q$ is any $j$-closed open, then $\mu(Q)$ is a $j$-closed lower real.
			...
			By \cref{cor.closed_sum},\dis{Need $j$ to commute with $\exists (q:\tqq)$ for this ($j$ decidable is not enough).} a sum of lower reals is $j$-closed if and only if each summand is. Since $j\bot$ and $\top$ are $j$-closed, so are the associated constant predicates $\ind{j\bot}$ and $1=\ind{\top}$, and hence the right-hand side of \eqref{j_modularity} is $j$-closed. Hence the left-hand side is too, and it follows that $\mu(Q) = j \mu(Q)$, as was to be shown.
