...
Recall that $\inc : \prop \to [0,1]$ is the inclusion of propositions into the unit interval, given by $\ind{P}(q) := (q<0)\vee((q<1)\wedge P)$. Recall also from \cref{cor.top_on_one} that $\prop$ is the unique topology on the one-point space $\{*\}$.

\begin{proposition}
	\label{prop.existence_val_on_1}
	There exists a valuation on the one-point space $\{*\}$, namely the indicator function $\inc : \prop \to [0,1]$.
\end{proposition}
...
By \cref{prop.existence_val_on_1} it suffices to show that an arbitrary valuation $\mu : \prop \to [0,1]$ on $\{*\}$ satisfies $\mu(P) = \ind{P}$ for all $P : \prop$. One direction is easy, namely
	\begin{equation}\label{eqn.ind_less_val}
		\ind{P} \le \mu(P).
	\end{equation}
	for all $P:\prop$. Indeed take $q<\ind{P}$. If $q<0$ then $q<\mu(P)$; otherwise $(q<1)\wedge P$, but then $\mu(P)=\mu(\top)=1$ because $P=\top$. 
...
\begin{proof}
\dis{This uses commutation of $j$ with addition of lower reals, which follows from ``axiom 4''.}
	...
	\begin{itemize}
		\item If $Q$ is any $j$-closed open, then $\mu(Q)$ is a $j$-closed lower real.
			...
			When applied to any rational $q$, the right-hand side of \eqref{j_modularity} is clearly $j$-closed, 
			\dis{I don't see this unless for some reason we're allowed to only use test rationals in [0,1].}hence so is the left-hand side. But by \cref{cor.closed_sum}, a sum of lower reals is $j$-closed if and only if each summand is, and it follows that $\mu(Q) = j \mu(Q)$, as was to be shown.
...
% ======== Chapter ========%
\chapter{Decidable modalities}

\tob{The following now only presents the definition of decidable modality and then consequences of it. So I think that this merits a separate treatment in its own section on decidable modalities. We still need to adjust the notation}

\begin{definition}[Decidable modality]
	\tob{rephrase this}
	For all $\pt : \Pts$, the modality $\At $ is \emph{locally decidable}. That is, for all $P : \prop$:
	\[\At P \: \lor \: (P \Rightarrow \At \bot).\]
\end{definition}


Similar to how a point of a locale is a completely prime filter on its frame of opens, we think of every $\At $ as specifying a point (although there is nothing in our axiomatics which requires the associated subtopos to be equivalent to $\Cat{Set}$). Then the above axiom is analogous to ``prime filter'', still missing the ``completely''. This is what we turn to next; similar to how preservation of $\lor$ is analogous to primality, it is a suitably type of commutation with $\exists$ which is analogous to complete primality.

\begin{proposition}[$\At $ commutes with $\vee$]
For all $\pt:\Pts$ and $P,Q:\prop$ we have
\[\At (P \lor Q) = \At P \lor \At Q.\]
\end{proposition}
\begin{proof}
The direction $(\At P \lor \At Q)\imp \At (P \lor Q)$ is straightforward. For the converse, assume $\At (P \lor Q)$. By \cref{ax.decidability} we may assume $P\imp\At \bot$ and $Q\imp\At \bot$. But then we have $\At \bot$, which in turn proves $\At P$.
\end{proof}

\begin{proposition}[$\At \bot$ is prime]
For any $\pt:\Pts$ and $P,Q:\prop$, we have
\[\big((P\wedge Q)\imp\At \false\big)\imp\big((P\imp\At \false)\vee(Q\imp\At \false)\big)\]
\end{proposition}
\begin{proof}
Assume $(P\wedge Q)\imp\At \false$. By \cref{ax.decidability} we may assume also that $\At P$ and $\At Q$, hence $\At (P\wedge Q)$ and finally $\At\bot$.
\end{proof}

\begin{proposition}[$\At$ is quasi-closed]
For any $\pt:\Pts$ and $P:\prop$ we have
\[\At P=\big((P\imp \At\bot)\imp\At\bot\big).\]
\end{proposition}
\begin{proof}
The forward direction would hold for any modality $j$ in place of $\At$, and the backward direction follows directly from \cref{ax.decidability}.
\end{proof}

We now turn to the treatment of the existential quantifier.

\begin{lemma}[The $j$-existential quantifier as a double $j$-negation]
	Let $j$ be a decidable modality, $X$ any type and $P : X \to \prop$ a predicate. Then
	\[
		j \left( \exists x : X . P(x) \right) \quad \Longleftrightarrow \quad \left( \forall x : X. P(x) \imp j \bot \right) \imp j \bot.
	\]
\end{lemma}
...
In this section, we introduce a collection of modalities which internally express the idea that our topos has enough points. We fix a type $\Pts$ as indexing a family of modalities $\At : \prop \to \prop$. (The reason behind our notation $k : \Pts$ will become clear when we consider hyperspaces, in which case $k$ represents a compact set.)

Our primary intended semantics is that our topos is $\shv(X)$ for some topological space $X$, and that every modality $\At{k}$ is the quasi-closed modality associated to the open set $X \setminus \overline{k}$. More precisely, consider the underlying set $X$ of the space, and consider the associated constant presheaf on $(X,\Op_X)$, which assigns the set $X$ to each open $U\in\Op_X$. We take $\Pts$ to be the sheafification of this presheaf.
