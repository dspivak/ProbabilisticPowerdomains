...
\begin{definition}
	A family $(r_\pt)_{\pt : \Pts}$ with $r_\pt \in \tlrr_{\At}$ for every $\pt$ is \emph{lower semicontinuous} if there exists a function $U\colon\tqq\to\prop$ such that for all $q : \tqq$ and $\pt : \Pts$,
	\begin{equation}\label{eqn.witness}
		r_\pt(q) \Leftrightarrow \At U(q).
	\end{equation}
	We call $U$ the \emph{witnessing cut}.
\end{definition}
...
\begin{equation}
	...
	r_\pt \le \At r_{\pt'},
\end{equation}
since $\At U(q) \imp \At \At[\pt'] U(q)$ for any $q:\tqq$.
