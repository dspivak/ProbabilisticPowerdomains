...
We want to show order cancellativity, \cref{prop.order_cancel}. To do so we follow the proof of Simon Henry \cite{Henry}.%\url{https://mathoverflow.net/questions/108029/simplification-in-semi-continuous-real?rq=1}
\begin{lemma}
...
\begin{proposition}[Order cancellativity]\label{prop.order_cancel}
Suppose $r_0,r_1,r_2:\trr$ are lower reals and that $r_0$ is bounded in the sense that $r_0\leq q_0\geq r_0$ for some $q_0:\tqq$. If $r_0+r_1\geq r_0+r_2$ then $r_1\geq r_2$.
\end{proposition}
\begin{proof}
...
\begin{proof}
	Since $jP \land (P \Rightarrow j\bot)$ is equivalent to $j\bot$, applying the modularity law to $jP$ and $P \Rightarrow j\bot$ results in
	\[
		\mu(jP) + \mu(P \Rightarrow j\bot) = \mu(j\bot) + \mu(jP \lor (P \Rightarrow j\bot)).
	\]
\end{proof}
