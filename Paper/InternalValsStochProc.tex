...
\addbibresource{InternalValsStochProc.bib}
...
\newcommand{\Vals}{\mathscr{V}}	% type of valuations
...
It is known that (continuous) valuations from a construction in geometric logic~\cite[Proposition~5]{vickers_integrals}. This implies that they behave well with respect to pullback along geometric morphisms. In terms of \Cref{main_external}, this means that every internal valuation defines a family of external valuations $(\mu_\pt)_{\pt \in \Pts}$, and it may be possible to argue from geometricity alone that these satisfy the compatibility and lower semicontinuity conditions of \Cref{main_external}. However, as far as we know, currently available methods in geometric logic do not give us a purely abstract way of getting from the family $(\mu_\pt)_{\pt \in \Pts}$ to the internal valuation $\mu$~\cite[Section~5]{vickers_geometric_logic}.
...
A \emph{lower nonnegative real} to be a function $r_>\colon\tqq\to\prop$ satisfying the following axioms:%
...
More concretely, this means that a $j$-local internal space consists of an object $X$ and an openness predicate $\Open_X^j : (\prop_j)^X \to \prop_j$, where $\prop_j$ is the object of $j$-closed propositions. This openness predicate needs to satisfy the obvious $j$-versions of the conditions above. Let $\Op[X]_j\ss(\prop_j)^X$ denote the $j$-closed subobject classified by $\Open^X_j$. Reducing the number of occurrences of $j$ as much as possible, these conditions are as follows:
...
\tob{mention comparison with Garner's ionads}
...
\begin{lemma}
	...
	For $\pt \in \Pts$, the type $\At \Open(E_p)$ has the following semantics:
	\[
...
\begin{proof}
	...
	\[
		\mu_{\pointwise} : \Pts \times \Open(E) \longrightarrow \tlrr
	\]
	such that every $\mu_{\pointwise}(\pt,-) : \Open(E) \to \tlrr$ is an $\At[\pt]$-local valuation, and such that the compatibility and lower semicontinuity conditions hold as $\pt$ varies.
	\tob{in progress}
\end{proof}
