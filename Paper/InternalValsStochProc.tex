...
\begin{axiom}[Local decidability]\label{ax.decidability}
	For all $\pt : \Pts$, the modality $\At{\pt}$ is \emph{locally decidable}. That is, for all $P : \prop$:
	\[\At{\pt}P \: \lor \: (P \Rightarrow \At{\pt} \bot).\]
\end{axiom}
...
\begin{proposition}[$\At{\pt}$ commutes with $\vee$]
For all $\pt:\Pts$ and $P,Q:\prop$ we have
\[\At{\pt}(P \lor Q) = \At{\pt} P \lor \At{\pt} Q.\]
\end{proposition}
\begin{proof}
The direction $(\At{\pt} P \lor \At{\pt} Q)\imp \At{\pt}(P \lor Q)$ is straightforward. For the converse, assume $\At{\pt}(P \lor Q)$. By \cref{ax.decidability} we may assume $P\imp\At{\pt}\bot$ and $Q\imp\At{\pt}\bot$. But then we have $\At{\pt}\bot$, which in turn proves $\At{\pt}P$.
\end{proof}
\begin{axiom}
