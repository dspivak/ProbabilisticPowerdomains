...
\section{Axioms for toposes with enough points}
\label{axioms_soundness}

In this section, we introduce a collection of modalities which express the idea that a given topos has enough points internally. To this end, we fix a type $\pts$ and a function $\at[]\colon\pts\to\decmod$, where $\decmod$ is the object of decidable modalities; see \cref{def.decidable_modality}. The idea is that $\pts$ is an object which (internally) indexes the points, and for every $\pt \in \pts$, the modality $\at : \prop \to \prop$ checks whether a given proposition holds at $\pt$. 
while this type of setup is conceptually reminiscent of Garner's \emph{ionads}~\cite{garner2012ionads}, we currently do not know what the technical relation is.
...
\begin{equation}\label{eqn.j_k}
	...
	\end{cases}
\end{equation}
If $\pt\in U$ then $j_\pt(U)=T$ and conditions 1, 2, and 4 are trivially satisfied. Supposing now that $\pt\not\in U$ then $U\subseteq \{\pt\}^c$ and hence $U\subseteq j_\pt(U)$, giving condition 1. Since $\pt\not\in(\{\pt\}^c)^\circ=j_\pt(U)$, we also have condition 2. Since $U\subseteq j_\pt(\varnothing)$, we have $T=U^c\cup U\subseteq (U^c\cup j_\pt(\varnothing))$ so we also have condition 4. For condition 3, if $\pt\in U\cap V$ then both sides are $T$; if $\pt\not\in U\cap V$ then $\pt\not\in U$ or $\pt\not\in V$ in which case both sides are $(\{\pt\}^c)^\circ$. Thus \eqref{eqn.j_k} satisfies the necessary conditions, and we have defined our sheaf homomorphism $\church{\at[]}$ from \eqref{eqn.at_semantics}.
...
\begin{soundproof}
Take $U\subseteq T$ and suppose $U\forces\forall (\pt : \pts). \at P$. For each point $\pt\in T$, we have $U\forces\at P$ iff either $\pt\in P$ or $U\subseteq\{\pt\}^c$. In particular for every $\pt\in U$ we have $\pt\in P$; hence $U\subseteq P$ which gives $U\forces P$ as desired.
\end{soundproof}
...
\begin{proposition}
	For each one of \Cref{ax.local_dec,ax.enough_pts,ax.N_flabby}, there is a topos which does not satisfy the axiom while satisfying the other two.
\end{proposition}
...
%	The proof will be complete upon showing that the double negation modality fails \Cref{ax.N_flabby} globally. To this end, consider the globally defined predicate $P$ where $\church{P} : \church{N} \to \church{\prop}$ is the unique sheaf map induced by the presheaf map out of the constant presheaf $\nn$ which takes every $n \in \nn$ to the open set $\{n\}$. Then $\nn \forces \exists n. P(n)$, and therefore $B \forces \lnot \lnot \exists n. P(n)$ by density of $\nn \subseteq B$. 
%
%However, if $B \forces \exists n . \lnot\lnot P(n)$ were to hold as well, then there would need to be a covering of $B$ by opens $(U_n)_{n \in \nn}$ such that $U_n \forces \lnot\lnot P(n)$. Then finitely many of those are enough by compactness. But since every $n \in \nn$ is an isolated point of $B$, the concrete form of $P$ shows that we can have $m \in U_n$ only if $m = n$. Therefore finitely many $U_n$ cannot cover $B$, contradiction.
...
\part{Constructive theory of valuations}


\section{Background on lower nonnegative reals}

We assume familiarity with sheaves, toposes, their internal logic, modalities and basics of geometric logic.
...
The number system in which valuations land is the lower nonnegative reals. We therefore introduce them here and discuss their relevant properties.

\begin{definition}[Lower nonnegative reals]\label{def.lower_nn_reals}
A \emph{lower nonnegative real} is a function $r_>\colon\tqq\to\prop$ satisfying the following axioms:%
...
\end{enumerate}
Accordingly, the type of lower nonnegative reals, denoted $\tlrr$, is
\[
...
\begin{definition}[Order on lower nonnegative reals]
...
\end{equation}
We also write $r'\leq r$ to denote $r\geq r'$.
\end{definition}

It is straightforward to show that $\geq$ is indeed a partial order relation on $\tlrr$ (in the internal logic). We therefore consider $\tlrr$ as a partially ordered object from now on.

\begin{definition}[Addition of lower nonnegative reals]\label{def.sum_lr}
Let $r_1,r_2:\tlrr$ be lower nonnegative reals. Define their \emph{sum}, denoted $(r_1+r_2)_>:\tqq\to\prop$ to be the predicate
...
\end{proposition}
\begin{proof}
	The pullback functor $f^{-1}\colon\pow(Y)\to\pow(X)$ preserves limits and colimits, so any of the conditions 1,3,4,5 that holds for $\mu$ holds for $f_*\mu$ also. For 2, suppose $U\imp V$. Then $f^{-1}(U)\imp f^{-1}(V)$, so $\mu(f^{-1}(U))\leq\mu(f^{-1}(V))$, proving the result.
\end{proof}
...
\part{Pointwise structure of internal valuations}
...
\begin{enumerate}
	\item Suppose that the open set corresponding to $\varphi\in\prop$ is $U\in\Op[X]$. Then $V\forces\varphi$ iff $V\subseteq U$.
  ...
  \item $U\forces \varphi\imp\psi$ iff whenever $V\forces\varphi$ holds for $V\subseteq U$, then also $V\forces\psi$ holds.
  \item $U\forces\neg\varphi$ iff whenever $V\forces\varphi$ holds for $V\subseteq U$, then $V=\varnothing$.
  ...
  \item $U\forces\forall (a:A).\varphi(a)$ iff for every $V\subseteq U$ and $a\in A(V)$ it holds that $V\forces \varphi(a)$.
\end{enumerate}
